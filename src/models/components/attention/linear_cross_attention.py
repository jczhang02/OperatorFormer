...
__all__ = ["CrossLinearMultiHeadAttention"]


class CrossLinearMultiHeadAttention(nn.Module):
    def __init__(
        self,
        ...
        super(CrossLinearMultiHeadAttention, self).__init__()
        ...
        self.q_projs = nn.Linear(in_features=d_model, out_features=d_model * nhead, bias=bias)
        self.kv_projs = nn.ModuleList(
            [deepcopy(nn.Linear(in_features=d_model, out_features=d_model * nhead, bias=bias)) for _ in range(2)]
        )
    ...
    def forward(
        ...
        k: Tensor
        v: Tensor
        q = rearrange(self.q_projs(query_emb), "b  n (h d) -> b h n d", h=self.nhead)
        k, v = [rearrange(kv_proj(input_emb), "b n (h d) -> b h n d", h=self.nhead) for kv_proj in self.kv_projs]
        q, k, v, mask = self._normalization(q, k, v, mask)

        q, k, v = self._positional_encoding(q, k, v, query_pos, input_pos)
    ...
    def _positional_encoding(
        ...
        assert query_pos is not None and input_pos is not None, "`pos` is None. Please pass in `pos` argument."
        if self.pos_config is None:
            return q, k, v

        if self.pos_config.method == "RoPE":
            query_freqs: List[Tensor] = self.pos_encoding(query_pos)
            input_freqs: List[Tensor] = self.pos_encoding(input_pos)
            query_freqs = [repeat(freq, "b n d -> b h n d", h=self.nhead) for freq in query_freqs]
            input_freqs = [repeat(freq, "b n d -> b h n d", h=self.nhead) for freq in input_freqs]

            q = _apply_rope_emb(q, query_freqs)
            k = _apply_rope_emb(k, input_freqs)
        elif self.pos_config.method == "cat":
            query_pos = repeat(query_pos, "b n d -> b h n d", h=self.nhead)
            input_pos = repeat(input_pos, "b n d -> b h n d", h=self.nhead)

            q = torch.cat([query_pos, q], dim=-1)
            k, v = [torch.cat([input_pos, input], dim=-1) for input in (k, v)]
    ...
    def _reset_parameters(self, init_config: ParamInitConfig) -> None:
        ...
        for p in chain(self.q_projs.parameters(), self.kv_projs.parameters()):
            if p.ndim > 1:
